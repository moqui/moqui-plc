...
In the `moqui-plc` checkout, repository `AGENTS.md` is the automatic bootstrap:
it resolves `agent-skills/CURRENT_SESSION`, loads the session state and routes
component-specific work to the Math, Device, Gateway and PLC knowledge
references. The active session carries context only; it does not authorize an
agent to resume old mutations without a current user request. Version durable
state and reviewed inputs, but keep ZIP exports, credentials, databases, runtime
logs and duplicate source snapshots out of Git.
